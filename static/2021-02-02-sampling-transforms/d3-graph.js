...
// d3 = require("d3@6");
function tree(data, target_div_class) {
	height = 600;
	width = 600;
	...
	for(var key in data)
	{
		node = {name: key, children: data[key]};
		datanodes.push(node);
	}
	treedata = {name: "inputs", children: datanodes};
	...
	function update(source) {
		...
		nodeEnter.append("text")
			...
			.text(d => 'distance' in d.data ? d.data.distance.toFixed(2) : "")
			.clone(true).lower()
			.attr("stroke-linejoin", "round")
			.attr("stroke-width", 3)
			.attr("stroke", "white");
